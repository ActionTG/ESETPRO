        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Aa]dd$") or text:match("^فعال شو$") then
          local adding = function(extra, result)
            ...
            if database:get("bot:enable:" .. msg.chat_id_) then
              if not database:get("lang:gp:" .. msg.chat_id_) then
                send(msg.chat_id_, msg.id_, 1, "• گروه " .. result.title_ .. " از قبل در لیست گروه های امن ربات میباشد !", 1, "md")
              else
                send(msg.chat_id_, msg.id_, 1, "• Group " .. result.title_ .. " is *Already* in *Management* list !", 1, "md")
              end
            else
              if database:get("lang:gp:" .. msg.chat_id_) then
                send(msg.chat_id_, msg.id_, 1, "• Group " .. result.title_ .. [[
...
If you *want* to get the bot phone number , send command *Botphone* .]], 1, "md")
                ...
                send(msg.chat_id_, msg.id_, 1, "• گروه " .. result.title_ .. "گروه به لیست گروه های امن ربات افزوده شد !", 1, "md")
              end
              local fname = result.first_name_ or ""
              local lname = result.last_name_ or ""
        ...
        if is_admin(msg.sender_user_id_) and idf:match("-100(%d+)") and text:match("^[Rr]em$") then
          ...
          if not database:get("bot:enable:" .. msg.chat_id_) then
            if database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "• Group " .. result.title_ .. " is *Not* In *Management* List !", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "• گروه " .. result.title_ .. " در لیست مدیریتی ربات نیست !", 1, "md")
            end
          else
            if database:get("lang:gp:" .. msg.chat_id_) then
              send(msg.chat_id_, msg.id_, 1, "• Group " .. result.title_ .. " Has Been *Removed* From *Management* List !", 1, "md")
            else
              send(msg.chat_id_, msg.id_, 1, "• گروه " .. result.title_ .. " از لیست مدیریتی ربات حذف شد !", 1, "md")
            end
            database:del("bot:charge:" .. msg.chat_id_)
            database:del("bot:enable:" .. msg.chat_id_)
            database:srem("bot:groups", msg.chat_id_)
        ...
        if is_admin(msg.sender_user_id_) and (text:match("^[Bb]otphone$") or text:match("^شماره ربات$")) then
		  ...
		  if is_admin(msg.sender_user_id_) and (text:match("^[Ss]are$") or text:match("^برنامه نویس$")) then
          local botphone = function(extra, result)
            if result.phone_number_ then
              local phone = result.phone_number_
              local fname = result.first_name_ or ""
              local lname = result.last_name_ or ""
              sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 989036091185, "Mr", "Rad", 495059735)
            end
          end
